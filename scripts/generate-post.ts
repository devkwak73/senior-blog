...
1. 이 글은 의학적 진단·처방이 아닌 **일반적인 생활 건강 정보**임을 명확히 할 것
2. 구체적인 약 이름, 복용량, 치료법을 직접 권유하지 말 것 (예: "○○약을 드세요" 금지)
3. 증상이 있으면 반드시 **"가까운 병원에서 전문의 상담을 받으세요"**라고 안내할 것
4. "~하면 낫습니다", "~하면 치료됩니다" 같은 확정적 표현 금지 → "~에 도움이 될 수 있어요" 수준으로 작성
5. 민간요법, 검증되지 않은 건강식품 효능을 사실처럼 쓰지 말 것
6. 건강·의학 주제 글의 마지막에 반드시 아래 면책 문구를 포함할 것:
...
[SEO 최적화 규칙]
1. 제목의 핵심 키워드(${topic.keywords})를 글 본문 첫 100자 안에 자연스럽게 포함
2. h2 소제목에도 SEO 키워드를 자연스럽게 배치 (최소 2개 h2에 키워드 포함)
3. 본문 전체에서 핵심 키워드가 3~5회 자연스럽게 반복
4. 관련 롱테일 키워드와 동의어를 함께 사용 (예: 관절염 → 퇴행성관절염, 무릎통증)
5. 첫 단락을 검색 스니펫에 적합하도록 질문+답변 형태로 작성
